...
from vision_engine_local import (
    ...
    detailed_page_summary_from_extract_and_visual as _page_detail_fn,
    check_ollama_models,
...
/* Verbatim PDF helper (see Per-segment detail) */
p.sb-verbatim-help {
  font-size: var(--body-sm) !important;
  color: var(--text2) !important;
...
.sb-verbatim-pdf {
  ...
  overflow: auto;
  box-shadow: inset 3px 3px 10px var(--sd), inset -2px -2px 8px rgba(255,255,255,0.75);
...
# ── Ollama model list (cached; used for defaults & sidebar) ───────────────────
@st.cache_data(ttl=30)
def _get_ollama_models():
    return check_ollama_models()


_ollama_info = _get_ollama_models()
...
_defaults: dict = {
    ...
    "process_idx":      0,
    "frame_summaries":  [],
    "frame_verbatim_extracts": [],  # parallel to frame_summaries: raw PDF text per page (PDF only)
    "frame_labels":     [],
    "is_video":         False,
    "media_path":       None,
    "run_dir":          None,
    ...
    "max_images_val":   5,
    "video_frames_val": 8,
    "vision_model":     pick_default_vision_model(_ollama_info),
    "text_model":       pick_default_text_model(_ollama_info),
...
def _append_current_file_sources() -> None:
    ...
    verbatim: list[tuple[str, str]] = [
        (Path(ps).name, vx)
        for ps, vx in zip(ss.image_paths, ss.frame_verbatim_extracts)
        if vx and str(vx).strip()
    ]
    ss.batch_sources.append(
        {
            "name": src_name,
            "segments": segments,
            "verbatim_segments": verbatim,
            "transcript": tr,
        }
    )
...
if _nav == "upload":
    ...
    with col_right:
        ...
        # Uploader allows ".mpeg" too; treat it like other video types for this control.
        _video_exts = set(VIDEO_EXTS) | {".mpeg"}
        _has_video_in_selection = has_file and any(
            Path(uf.name).suffix.lower() in _video_exts for uf in files_sel
        )
        ...
        video_max_frames = st.slider(
            ...
            label_visibility="collapsed",
            key="sl_frames",
            disabled=not _has_video_in_selection,
        )
        st.markdown(
            f'<div style="display:flex;justify-content:flex-end;margin-top:-0.55rem;">'
        ...
        if has_file and not _has_video_in_selection:
            st.caption("Enabled when at least one uploaded file is a video (MP4, MOV, …).")
...
if _nav == "upload" and _done_like and _has_segments:
    ...
    if ss.batch_sources:
        ...
        for src in ss.batch_sources:
            sname = src["name"]
            verbatim_map = {
                vl: vt for vl, vt in (src.get("verbatim_segments") or []) if str(vt).strip()
            }
            for label, summary in src.get("segments") or []:
                with st.expander(f"{sname} — {label}"):
                    ...
                    st.markdown("**Summary**")
                    st.write(summary)
                    raw = verbatim_map.get(label)
                    if raw:
                        cap = raw[:120000] + ("…" if len(raw) > 120000 else "")
                        with st.expander("Original PDF text (exact extract)", expanded=False):
                            st.markdown(
                                '<p class="sb-verbatim-help">Exact wording from this page’s PDF text layer—the same '
                                "string the app uses for grounding and for Ask/Q&amp;A. It is not a rewrite; open "
                                "this when you need to verify citations, numbers, or phrases.</p>",
                                unsafe_allow_html=True,
                            )
                            st.markdown(
                                f'<div class="sb-verbatim-pdf"><pre>{html.escape(cap)}</pre></div>',
                                unsafe_allow_html=True,
                            )
...
if ss.stage == "batch_finalize":
    ...
    if ss.should_terminate:
        ...
        extracted_txt = ""
        if Path(ss.media_path).suffix.lower() == ".pdf":
            try:
                extracted_txt = extract_pdf_page_text(Path(ss.media_path), ss.process_idx)
            except Exception:
                extracted_txt = ""
        visual_part = _vision_fn(
            p,
            source_id=str(p),
            model_name=ss.vision_model,
            page_extracted_text=extracted_txt if extracted_txt.strip() else None,
        )
        if extracted_txt.strip():
            ss.frame_verbatim_extracts.append(extracted_txt.strip())
            try:
                page_body = _page_detail_fn(
                    extracted_txt,
                    visual_part,
                    source_id=str(p),
                    model_name=ss.text_model,
                ...
                page_body = (
                    f"{visual_part}\n\n---\n\n"
                    f"*Could not run the text model for a detailed rewrite ({exc}). "
                    "Showing the raw PDF extract below.*\n\n"
                    f"{extracted_txt.strip()[:80000]}"
                )
            ss.frame_summaries.append(page_body)
        else:
            ss.frame_verbatim_extracts.append("")
            ss.frame_summaries.append(visual_part)
